...
We will see a quick setup to configure the ``wikiless`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``wikiless`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``wikiless`` formulas under ``/srv/salt/wikiless/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/wikiless/parameters/dns:domain/
    ln -s dns:domain /srv/salt/wikiless/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/wikiless/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/wikiless/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``wikiless`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``wikiless`` formulas under ``/srv/salt/wikiless/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/wikiless/parameters/roles
...
We create a configuration for the role ``wikiless/server`` in ``/srv/salt/wikiless/parameters/roles/wikiless/server.yaml``:
...
We create another configuration for the role ``wikiless/client`` in ``/srv/salt/wikiless/parameters/roles/wikiless/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles wikiless/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles wikiless/client
...
.. code-block:: console
    ...
    mv /srv/salt/wikiless/parameters/dns:domain/  '/srv/salt/wikiless/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-wikiless-as-json:
      test.nop:
        - name: {{ wikiless | json }}
...
Here is an example based on `template-formula/wikiless/config/file.sls`_:
...
.. code-block:: sls
    ...
    wikiless-config-file-file-managed:
      file.managed:
        - name: {{ wikiless.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='wikiless-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ wikiless.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``wikiless`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ wikiless['winner'] }}
...
.. _template-formula/wikiless/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/wikiless/config/file.sls
